        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="p-5 bg-slate-900/90 border border-slate-800 backdrop-blur-xl rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl">
            <div className="flex items-center gap-3.5">
              <div className="w-10 h-10 rounded-2xl brand-gradient flex items-center justify-center text-slate-950 font-black shadow-lg shadow-[#38b6ff]/20">
                <Layers className="w-5 h-5" />
              </div>
              <div>
                <h1 className="text-lg font-black text-white flex items-center gap-2">
                  <span>TradeSlot Interactive Portal</span>
                  <span className="px-2.5 py-0.5 rounded-full bg-[#38b6ff]/10 text-[#38b6ff] border border-[#38b6ff]/20 text-[10px] uppercase font-mono font-bold">
                    Authenticated Slot
                  </span>
                </h1>
                <p className="text-xs text-slate-400 mt-0.5">
                  Role:{" "}
                  {userRole ? (
                    <strong className="text-[#38b6ff] uppercase font-bold">
            ...
            {currentUser ? (
              <div className="flex items-center gap-3 px-3.5 py-2 bg-slate-950 border border-slate-800 rounded-2xl">
                <div className="w-7 h-7 rounded-full bg-[#38b6ff]/10 border border-[#38b6ff]/20 flex items-center justify-center text-[#38b6ff] text-xs font-bold">
                  {currentUser.firstName?.[0] || "U"}
                </div>
                <div className="text-left text-xs">
                  <div className="font-bold text-white flex items-center gap-1">
                    <span>{currentUser.firstName} {currentUser.lastName}</span>
                    <ShieldCheck className="w-3.5 h-3.5 text-[#8c52ff]" />
                  </div>
                  <div className="text-[10px] text-[#38b6ff] font-mono">
                    {currentUser.role}
                  </div>
                </div>
              </div>
              ...
              <Link
                href="/login"
                className="px-4 py-2.5 btn-primary text-slate-950 font-bold rounded-2xl text-xs transition-all flex items-center gap-1.5 shadow-lg shadow-[#38b6ff]/20 self-start sm:self-auto cursor-pointer"
              >
          ...
          <main className="min-h-[500px]">
            {isLoading ? (
              <div className="p-12 text-center bg-slate-900/40 border border-slate-800 rounded-3xl space-y-3">
                <div className="w-8 h-8 border-2 border-[#38b6ff] border-t-transparent rounded-full animate-spin mx-auto" />
              ...
              <div className="p-12 text-center bg-slate-900/60 border border-slate-800 rounded-3xl space-y-4 shadow-2xl">
                ...
                <Link
                  href="/login"
                  className="inline-flex items-center gap-2 px-6 py-3 btn-primary text-slate-950 font-bold rounded-2xl text-xs transition-all shadow-lg shadow-[#38b6ff]/20 cursor-pointer"
                >
                  <LogIn className="w-4 h-4" />
